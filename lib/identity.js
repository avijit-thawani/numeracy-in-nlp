...
export const resolveIdentity = (config, event) => {
  ...
  const fromRepoName = repo?.name ? titleFromRepoName(repo.name) : null;

  const title = config.title?.trim() || fromRepoName || "My Living Survey";
  const description =
    config.description?.trim() ||
    repo?.description?.trim() ||
    "A living literature survey. Papers are added from `papers.txt`; suggestions update themselves.";
