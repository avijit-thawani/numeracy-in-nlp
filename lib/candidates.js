...
import { fetchCitationIds, fetchReferenceIdsFor, fetchPapers } from "./semanticScholar.js";
 ...
 * batch requests.
 */
export const buildCandidates = async ({ papers, limit, dismissedIds, seeded, algorithm = {} }) => {
  ...
  const forward =
    algorithm.forward !== false && seedIds.length ? tally(await fetchCitationIds(seedIds)) : new Map();
  ...
  let globalCitations = new Map();
  let backward = new Map();
  if (algorithm.backward !== false && seedIds.length) {
    const refs = await fetchReferenceIdsFor(seedIds);
    globalCitations = refs.citationCounts;
    backward = tally(refs.edges);
  }

  log.stat("papers citing this survey", forward.size);
  log.stat("papers cited by this survey", backward.size);

  const minCount = Math.max(2, algorithm.minCount ?? 2);
  ...
  const orderById = new Map(ranked.map(([id], i) => [id, i]));

  const scored = found.map((paper) => {
    const hit = rankById.get(paper.id);
    const seed = seededById.get(paper.id);
    let why = "related";
    if (hit?.dir === "forward") why = `cites ${hit.count} here`;
    else if (hit?.dir === "backward") why = `cited by ${hit.count} here`;
  ...
  // Keep the interleaved ranking worked out above; bibliography entries trail.
  scored.sort((a, b) => {
    const ai = orderById.has(a.id) ? orderById.get(a.id) : Infinity;
    const bi = orderById.has(b.id) ? orderById.get(b.id) : Infinity;
    return ai - bi || b.citationCount - a.citationCount;
  });
