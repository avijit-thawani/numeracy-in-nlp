...
const papersTable = (papers) => {
  if (!papers.length) {
    return [
      "_No papers yet._ Add some links to [`papers.txt`](papers.txt) and commit, and the table below fills itself in.",
    ];
  }
...
const candidatesTable = (candidates) => {
  if (!candidates.length) {
    return [
      "_None yet._ Suggestions appear once several papers in this survey share a citing paper, which usually needs around ten papers here.",
    ];
  }
